      ...
      //      return std::forward<const data_t>(  (data_t&&) d);
       ...
       //     return std::move(d);
        ...
        auto perform_pre(const typename  box<Ty,Aggregate>::agg_t  &a,  data_t&& d) const {
            return _fpre ? _fpre(std::move(d),a) : std::move(d);
        }

        auto perform_post(const typename  box<Ty,Aggregate>::agg_t  &a, data_t&& d) const {
            return _fpost ? _fpost( std::move(d),a) : std::move(d);
        }
        ...
        auto perform_choice(const typename  box<Ty,Aggregate>::type &a, data_t&& d) const
                {
                       ...
                       assert(test_ <=1);
                        return   a.index()==0  ?
                        o(  std::get< Ty >(a) , std::forward< data_t&&>(d) )
                        :
                        o(  std::get< typename box<Ty,Aggregate>::agg_t >(a) , std::forward< data_t&&>(d) );
                };
...
/*
        auto operator()(const box<Ty,Aggregate> &a, const data_t&& d) const->data_t{
            return  perform_choice(a._value, std::move(d) );    ///(*this)(a._value, std::move( d));

        };*/

        auto operator()(const typename box<Ty,Aggregate>::type &a, data_t&& d) const->stream_box::data_t;
    ...
    ::operator()(const typename box<Ty, Aggregate>::type &a,  stream_box::data_t&& d) const -> stream_box::data_t{
std::cout<<"-!- "<< a.index()<<".\t";
        auto t = a.index();
